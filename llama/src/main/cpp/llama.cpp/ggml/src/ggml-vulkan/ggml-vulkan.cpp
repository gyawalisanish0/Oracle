...
static vk_device ggml_vk_get_device(size_t idx) {
    ...
    if (vk_instance.devices[idx] == nullptr) {
        ...
        // Oracle: route through the dynamic dispatcher (loaded from the 1.2+
        // instance) rather than the linked C symbol, so the plugin links at API 24
        // and runs on pre-API-28 devices that still have a Vulkan 1.2 driver.
...
static void ggml_vk_print_gpu_info(size_t idx) {
    ...
    VULKAN_HPP_DEFAULT_DISPATCHER.vkGetPhysicalDeviceFeatures2(physical_device, &device_features2); // Oracle: dynamic dispatch (API-24 link compat)
...
static void ggml_vk_instance_init() {
    ...
    // Oracle: drop GPUs that lack 16-bit storage. ggml-vulkan hard-requires it and
    // otherwise throws "Unsupported device" deep in the model-load path, which
    // unwinds through the C backend boundary and crashes the process instead of
    // falling back. Filtering here means only usable GPUs are ever offered, so on
    // devices like the Adreno 610 llama just keeps everything on the CPU.
    {
        std::vector<size_t> usable_indices;
        for (size_t idx : vk_instance.device_indices) {
            ...
            VULKAN_HPP_DEFAULT_DISPATCHER.vkGetPhysicalDeviceFeatures2(devices[idx], &feats2);
            if (feats11.storageBuffer16BitAccess) {
                usable_indices.push_back(idx);
            } else {
                GGML_LOG_INFO("ggml_vulkan: skipping device %zu — no 16-bit storage (unsupported).\n", idx);
            }
        }
        vk_instance.device_indices = usable_indices;
        if (vk_instance.device_indices.empty()) {
            GGML_LOG_INFO("ggml_vulkan: No usable Vulkan devices (16-bit storage required).\n");
            return;
        }
    }
...
static bool ggml_vk_device_is_supported(const vk::PhysicalDevice & vkdev) {
    ...
    VULKAN_HPP_DEFAULT_DISPATCHER.vkGetPhysicalDeviceFeatures2(vkdev, &device_features2); // Oracle: dynamic dispatch (API-24 link compat)
